...
def compute_Rg(ns, traj_type):
	...
	if traj_type == 'AA':
		...
		for ts in ns.aa_universe.trajectory:
			gyr_aa[ts.frame] = ns.aa_universe.atoms[:len(ns.all_atoms)].radius_of_gyration(unwrap=True, backend=ns.mda_backend)
		...
		for ts in ns.aa2cg_universe.trajectory:
			gyr_aa_mapped[ts.frame] = ns.aa2cg_universe.atoms[:len(ns.cg_itp['atoms'])].radius_of_gyration(unwrap=True, backend=ns.mda_backend)
		...
		for ts in ns.cg_universe.trajectory:
			gyr_cg[ts.frame] = ns.cg_universe.atoms[:len(ns.cg_itp['atoms'])].radius_of_gyration(unwrap=True, backend=ns.mda_backend)
		...
		if not ns.cg_itp['atoms'][bead_id]['bead_type'].startswith('v'):  # bead is NOT a virtual site
			...
			for ts in ns.aa_universe.trajectory:
				traj[ts.frame] = ns.mda_beads_atom_grps[bead_id].center(
					ns.mda_weights_atom_grps[bead_id], unwrap=True, compound='group'
				...
				'''
    				...
    				MDanalysis的版本变更：下面函数的关键词pbc改成wrap了。所以swarmcg里面的pbc=None改成wrap=False或者unwrap=True(区别参考MD的center函数定义)
...
def eval_function(parameters_set, ns):
	...
	if os.path.isfile('mini.gro'):
		...
		if os.path.isfile('equi.gro'):
			...
			# grompp -- MD成功运行结束后，增加对结果轨迹周期性处理的过程，使用gmx自带的pbc工具处理周期性问题。
			# make traj whole and centered
			# if MD finished properly, we just check for the .gro file printed in the end
			if os.path.isfile('md.gro'):
				# pbc to choose the index group, 2 is the molecule even when there are ions (0 and 1 are System and Others)注意建模的时候：文件.top中分子的顺序必须按1、被优化分子.2、溶剂分子。3、离子这个顺序建模。
				gmx_cmd = f'echo 2 0 | {ns.gmx_path} trjconv -s md.tpr -f md.xtc -pbc mol -center -o md.xtc'
				return_code = exec_gmx(gmx_cmd)
				if return_code != 0:
					msg = (
						f"Gromacs 优化对象的轨迹周期性居中处理失败！\n"
					)
					raise exceptions.ComputationError(msg)
